#!/usr/bin/env python3
"""Process Claude Code session data into visualization-ready JSON."""
...
def process_all_data():
    """Process all Claude Code session data."""
    messages = []
    ...
    for project_dir in sorted(PROJECTS_DIR.iterdir()):
        ...
        # Read main session JSONL files (top-level only, skip subdirs)
    ...
    for msg in messages:
        ...
        if pdir in project_name_map:
            ...
            # Fallback: extract last segment from directory name heuristic
            parts = pdir.split("-")
...
def compute_stats(messages):
    ...
    # ── Insights (from /insights facets & session-meta) ──────────
...
if __name__ == "__main__":
    ...
    all_messages = process_all_data()
    print(f"  Found {len(all_messages)} total records")

    stats = compute_stats(all_messages)
    print(f"  {stats['overview']['totalSessions']} sessions")
    print(f"  {stats['overview']['totalMessages']} messages")
    print(f"  {len(stats['projects'])} projects")
    print(f"  {len(stats['tools'])} unique tools")
    print(f"  Date range: {stats['overview']['firstDate']} → {stats['overview']['lastDate']}")
